...
6. **Committee/hearing material.** Ingest agendas, schedules, testifier lists, written testimony/handouts, amendments, minutes, roll calls, bill summaries, and fiscal-note revisions with official dates and bill resolution. The archive metadata collector preserves the official posted date and attachment URL first; the follow-up PDF-content collector then fetches only those already-indexed official House PDF attachments in bounded batches, preserves the exact byte SHA-256/fetch timestamp/archive-page lineage, and stores extracted text as neutral context with `modelWeight=0`, `mechanicallyActionable=false`, and same-day ineligibility. The historical availability boundary remains the archive's official attachment-posted date; document/event dates found inside the PDF are never substituted for availability.
