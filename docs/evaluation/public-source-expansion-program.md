...
6. **Committee/hearing material.** Ingest agendas, schedules, testifier lists, written testimony/handouts, amendments, minutes, roll calls, bill summaries, and fiscal-note revisions with official dates and bill resolution. The archive metadata collector preserves the official posted date and attachment URL first. The follow-up PDF-content collector fetches only those already-indexed official House PDF attachments in bounded batches, preserves exact byte SHA-256/fetch timestamp/archive-page lineage, and stores extracted text as neutral context with `modelWeight=0` and `mechanicallyActionable=false`. The archive posted date proves the historical listing/link, not that bytes fetched today are identical to the historical bytes; current PDF content therefore remains `asOfEligible=false` until an independent archive capture or other exact historical-content proof establishes byte identity and availability. Document/event dates found inside the PDF are never substituted for availability.
