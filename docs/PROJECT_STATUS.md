...
The project should therefore reduce broad retrospective feature searching on the same 2021-26 floor-vote outcomes. The identified selection problem is now handled as a separate lifecycle research program: model bill advancement/floor access over the full introduced-bill universe, retain member-vote scoring only where an actual floor vote supplies a label, and evaluate the end-to-end source-chamber passage probability against the accepted introduction prior. This is a target/population correction rather than another feature hunt.
...
Two final pre-prospective experiments were then frozen before results were observed. A later audit found that the original historical Quick replay could use mutable current bill titles/persisted features in dated replay. The replay has therefore been replaced by `historical-quick-replay-v2`, which derives identity/features from dated official bill text, ignores unsafe persisted historical feature sets, excludes current companion metadata, and enforces stricter historical availability boundaries. The original v1 experiment record is preserved, but its exact retrospective effect sizes are superseded by the v2 robustness results below. Because the 2023-24 outcomes were already inspected, these v2 reruns are robustness audits rather than fresh independent validation.
...
1. **Lifecycle prospective validation.** Full-universe lifecycle phases P2-P7 are complete and frozen under #310. A post-freeze chronology-safe bill-language robustness screen found no proper-score gain from open-vocabulary words (validation selected lexical scale 0), but modest incremental signal from structural language cues beyond P5, with Brier/log-loss/calibration improvements in 2023-24 and smaller same-direction improvements in 2025-26. This remains retrospective development evidence only and does not alter P8 or serving. The P8 prospective plan and exact 2027 model package are also frozen before any 2027-28 data existed (model-content SHA-256 `abcf583153939d46aa021dccf2afe61d698ad4538a981059cdee265c03166a65`). P7 `substantive_vehicle_passage_v1` adds 651 direct-vehicle positives to the 654 strict bill-number passages (1,305 total), with frozen label SHA-256 `5b18378725c736dd4eceb37992fd4295cbd779e24e3cdd92123f28192ab75eec`. The unchanged accepted v4 ranking signal transfers to the secondary target (ROC-AUC 0.78341) but is materially under-calibrated because it was trained for the narrower strict target. No recalibration or serving change is authorized. P8 clean 2027-28 prospective validation is the remaining lifecycle phase.
...
### P1 — build the full introduced-bill lifecycle evaluation
