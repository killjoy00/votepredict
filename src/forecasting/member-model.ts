...
export interface MemberModelOptions { fallback?: number; partyPriorStrength?: number; memberPriorStrength?: number; maximumMemberHistoryWeight?: number; maximumAnalogueWeight?: number; minimumGlobalSupport?: number; }
...
function shrink(observed: RateEvidence | undefined, prior: number, priorStrength: number, maximumObservedWeight = Number.POSITIVE_INFINITY): number {
  const observedRate = rate(observed);
  if (observedRate === undefined) return prior;
  const observedWeight = Math.min(observed?.total ?? 0, maximumObservedWeight);
  const totalWeight = observedWeight + priorStrength;
  if (totalWeight <= 0) return prior;
  return (observedRate * observedWeight + priorStrength * prior) / totalWeight;
}
...
export function estimateMemberProbability(input: MemberProbabilityInput, options: MemberModelOptions = {}): MemberProbabilityResult {
  ...
  const maximumMemberHistoryWeight = options.maximumMemberHistoryWeight ?? Number.POSITIVE_INFINITY;
  ...
  if (partyPriorStrength < 0 || memberPriorStrength < 0 || maximumMemberHistoryWeight < 0 || maximumAnalogueWeight < 0 || minimumGlobalSupport < 0) throw new Error('model strengths cannot be negative');
  ...
  const effectiveMemberWeight = Math.min(support.member, maximumMemberHistoryWeight);
  let probability = shrink(input.memberHistory, partyProbability, memberPriorStrength, maximumMemberHistoryWeight);
  if (input.analogueYesRate !== undefined && support.analogue > 0) {
    ...
    const baseWeight = Math.max(1, memberPriorStrength + effectiveMemberWeight);
