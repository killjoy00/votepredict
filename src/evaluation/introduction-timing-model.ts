...
export interface IntroductionTimingObservation extends IntroductionObservation {
  ...
  initialDocumentAvailableAtIntroduction: boolean;
}
...
export function initialDocumentLeadKey(row: Pick<IntroductionTimingObservation, 'introducedOn' | 'initialDocumentOn' | 'initialDocumentAvailableAtIntroduction'>): string {
  if (!row.initialDocumentAvailableAtIntroduction) return 'unavailable-at-introduction';
  if (!row.initialDocumentOn) throw new Error('Introduction-available initial document is missing its posting date');
  ...
  if (days < 0) throw new Error('Initial document postdates introduction despite being marked available');
...
export function trainIntroductionTimingModel(
  ...
  for (const row of observations) {
    ...
    addStat(initialDocumentLeadStats, initialDocumentLeadKey(row), row.outcome);
  }
  return { model: 'intro-title-timing-eb-v3', titleModel, baseRate: titleModel.baseRate, legislativeYearStats, yearMonthStats, initialDocumentLeadStats, options: resolved };
}

export function predictIntroductionTimingModel(
  model: IntroductionTimingModel,
  row: Pick<IntroductionTimingObservation, 'sessionStart' | 'title' | 'introducedOn' | 'initialDocumentOn' | 'initialDocumentAvailableAtIntroduction'>,
): number {
  const titleProbability = predictIntroductionTitleModel(model.titleModel, row.title);
  const leadKey = initialDocumentLeadKey(row);
  const yearDelta = empiricalBayesDelta(model.legislativeYearStats.get(introductionLegislativeYearKey(row)), model.baseRate, model.options.timingPriorStrength);
  const monthDelta = empiricalBayesDelta(model.yearMonthStats.get(introductionYearMonthKey(row)), model.baseRate, model.options.timingPriorStrength);
  const leadDelta = empiricalBayesDelta(model.initialDocumentLeadStats.get(leadKey), model.baseRate, model.options.timingPriorStrength);
  return clamp(logistic(logit(titleProbability) + model.options.legislativeYearScale * yearDelta + model.options.yearMonthScale * monthDelta + model.options.initialDocumentLeadScale * leadDelta), model.options.probabilityFloor, model.options.probabilityCeiling);
}
...
export function evaluateIntroductionTimingModelChronologically(
  ...
  for (let index = 1; index < sessionStarts.length; index += 1) {
    ...
    for (const row of holdout) {
      predictions.push({ billId: row.billId, asOf: `${row.introducedOn}T12:00:00Z`, targetKind: 'source_chamber_passage', outcome: row.outcome, probability: predictIntroductionTimingModel(model, row), model: model.model });
    }
  }
  return predictions;
}
