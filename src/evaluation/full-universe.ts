...
export async function evaluateAuthoritativeFullUniverse(database: Queryable, codeSha: string | null = null) {
  ...
  const timingRows = result.rows.filter((row) =>
    ...
    && (row.initial_document_model_eligible === 'true' || row.initial_document_model_eligible === 'false')
  ...
  const timingObservations: IntroductionTimingObservation[] = timingRows.map((row) => {
    const initialDocumentAvailableAtIntroduction = row.initial_document_model_eligible === 'true';
    return {
      billId: row.bill_id,
      sessionSlug: row.session_slug,
      sessionStart: row.session_start,
      ...
      // Never expose a later posting date to the introduction-stage model. A late document
      // is represented only by the contemporaneous state that no initial document was yet available.
      initialDocumentOn: initialDocumentAvailableAtIntroduction ? row.initial_document_on! : null,
      initialDocumentAvailableAtIntroduction,
      outcome: row.outcome ? 1 : 0,
    };
  });
  ...
  return {
    metadata: {
      ...
      candidates: [
        ...
        'intro-title-timing-eb-v3: v1 title probability plus strongly shrunk first/second legislative-year, year-month, and initial-document availability/lead-time effects from exact official introduction metadata.',
      ],
      caveat: 'No current author or companion state is model eligible. A zero-engrossment document posted after introduction is represented only as not-yet-available; its future posting date is not exposed to the model. Initial bill body text is not used. No candidate in this evaluator changes production forecasts.',
    },
    observations: observations.length,
    timingCoverage: timingRows.length,
    unavailableInitialDocumentsAtIntroduction: timingRows.filter((row) => row.initial_document_model_eligible === 'false').length,
    counts,
