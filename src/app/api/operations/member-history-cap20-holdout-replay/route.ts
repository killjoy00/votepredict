...
import { evaluateMemberHistoryCap20HoldoutReplay } from '@/evaluation/member-history-cap20-holdout-replay';
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = await evaluateMemberHistoryCap20HoldoutReplay(pool, {
      cohort: body.cohort,
    ...
    console.error('Member history cap-20 holdout replay failed', message);
    return NextResponse.json({ error: 'Member history cap-20 holdout replay failed' }, { status: 500 });
  }
}
