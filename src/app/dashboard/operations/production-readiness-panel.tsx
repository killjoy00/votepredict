  ...
  return (
    <section className="readiness-panel" aria-label="Production readiness">
      ...
      <div className="readiness-grid">
        ...
        <article>
          ...
          <div>
            ...
            <small>{readiness.evidence.currentCampaignFinanceItems.toLocaleString()} finance · {readiness.evidence.campaignSiteItems} campaign-site · {readiness.evidence.memberPrimaryItems} member-primary · {readiness.evidence.publicNewsItems} news / {readiness.evidence.publicNewsMembers} members · {readiness.evidence.committeeEvidenceItems} committee votes / {readiness.evidence.committeeEvidenceMembers} members · {readiness.evidence.publicEvidenceMembers} web-covered members · {ageLabel(readiness.evidence.latestPublicEvidenceRun)}</small>
          </div>
        </article>
        <article>
      ...
      <div className="evidence-footnote">
        Public evidence is stored independently of serving model impact. Quick Evidence candidate corpus: <strong>{readiness.evidence.quickEvidenceCandidateItems} items across {readiness.evidence.quickEvidenceCandidateMembers} members</strong>. Committee-minute pipeline: <strong>{readiness.evidence.latestCommitteeEvidenceStatus ?? 'not verified'}</strong>, last run <strong>{ageLabel(readiness.evidence.latestCommitteeEvidenceRun)}</strong>. Serving mechanical public-evidence items: <strong>{readiness.evidence.mechanicallyActionableItems}</strong>. Curated legacy items: <strong>{readiness.evidence.currentCuratedItems}</strong>. Campaign-finance members: <strong>{readiness.evidence.campaignFinanceMembers}</strong>. Member-primary source members: <strong>{readiness.evidence.memberPrimaryMembers}</strong>. Latest news batch: <strong>{readiness.evidence.latestNewsInserted} inserted</strong>, <strong>{readiness.evidence.latestNewsFailures} failures</strong>, <strong>{readiness.evidence.latestNewsNoLeadMembers} no-lead members</strong>. Prospective capture began <strong>{ageLabel(readiness.evidence.prospectiveEvidenceSince)}</strong>.
      </div>
