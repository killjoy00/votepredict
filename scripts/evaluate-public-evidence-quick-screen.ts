...
async function main() {
  ...
  const response = await fetch('https://votepredict.vercel.app/api/operations/public-evidence-quick-screen', {
    method: 'POST',
    headers: { Authorization: `Bearer ${cronSecret}` },
    signal: AbortSignal.timeout(310_000),
  });
  const body = await response.text();
  if (!response.ok) throw new Error(`Public evidence Quick screen returned HTTP ${response.status}: ${body.slice(0, 1800)}`);
  const artifact = JSON.parse(body) as Record<string, unknown>;
  const output = resolve(process.env.VOTEPREDICT_PUBLIC_EVIDENCE_SCREEN_OUTPUT ?? 'artifacts/public-evidence-quick-screen.json');
