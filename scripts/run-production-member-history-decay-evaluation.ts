...
async function main(): Promise<void> {
  ...
  const outputPath = resolve(
    process.env.VOTEPREDICT_MEMBER_HISTORY_DECAY_OUTPUT
      ?? 'artifacts/member-history-decay-evaluation-v1.json',
  );
  ...
  const response = await fetch('https://vote.planitnow.us/api/operations/member-history-decay-evaluation', {
    method: 'POST',
    headers: { Authorization: `Bearer ${cronSecret}` },
    signal: AbortSignal.timeout(310_000),
  });
  if (!response.ok) {
    throw new Error(`Production member-history decay evaluation returned HTTP ${response.status}`);
  }
  const parsed = await response.json() as {
    schemaVersion?: string;
    metadata?: { codeSha?: string | null };
    decision?: { status?: string; selectedCandidate?: string | null };
  };
  ...
  console.log(JSON.stringify({
    ...
    decisionStatus: parsed.decision?.status ?? null,
    selectedCandidate: parsed.decision?.selectedCandidate ?? null,
  }));
}
