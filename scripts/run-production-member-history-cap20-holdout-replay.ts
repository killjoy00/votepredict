...
async function main(): Promise<void> {
  ...
  const cohortPath = process.env.VOTEPREDICT_CAP20_HOLDOUT_COHORT;
  const scorePath = process.env.VOTEPREDICT_CAP20_HOLDOUT_SCORE;
  if (!envPath || !cohortPath || !scorePath) throw new Error('Production env, frozen cohort, and frozen holdout score are required');
  const outputPath = resolve(
    process.env.VOTEPREDICT_CAP20_HOLDOUT_REPLAY_OUTPUT
      ?? 'artifacts/member-history-cap20-holdout-replay-v1.json',
  );
  ...
  const response = await fetch('https://vote.planitnow.us/api/operations/member-history-cap20-holdout-replay', {
    method: 'POST',
    headers: {
  ...
  if (!response.ok) throw new Error(`Production cap-20 holdout replay returned HTTP ${response.status}`);
  const parsed = await response.json() as {
    schemaVersion?: string;
    metadata?: { runtimeCodeSha?: string | null; maximumMemberHistoryWeight?: number; baselineValidation?: unknown };
    summary?: { baseline?: unknown; cap20?: unknown; deltaCapMinusBaseline?: unknown };
  };
  ...
  console.log(JSON.stringify({
    ...
    maximumMemberHistoryWeight: parsed.metadata?.maximumMemberHistoryWeight ?? null,
    baselineValidation: parsed.metadata?.baselineValidation ?? null,
    baseline: parsed.summary?.baseline ?? null,
    cap20: parsed.summary?.cap20 ?? null,
    deltaCapMinusBaseline: parsed.summary?.deltaCapMinusBaseline ?? null,
  }, null, 2));
}
