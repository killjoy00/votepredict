...
async function main(): Promise<void> {
  ...
  const outputPath = resolve(process.env.VOTEPREDICT_DEEP_PILOT_OUTPUT ?? 'artifacts/historical-deep-pilot-manifest.json');
  ...
  const response = await fetch('https://vote.planitnow.us/api/operations/historical-deep-pilot-manifest', {
    method: 'POST',
    headers: { Authorization: `Bearer ${cronSecret}` },
    signal: AbortSignal.timeout(310_000),
  });
  if (!response.ok) {
    throw new Error(`Production historical Deep pilot manifest returned HTTP ${response.status}`);
  }
  const parsed = await response.json() as {
    metadata?: Record<string, unknown>;
    cases?: Array<{ identifier?: string; occurredOn?: string; targets?: unknown[] }>;
  };
  ...
  console.log(JSON.stringify({
    ...
    targetsPerCase: parsed.cases?.map((item) => ({
