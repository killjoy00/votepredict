...
async function main(): Promise<void> {
  ...
  const response = await fetch('https://votepredict.vercel.app/api/operations/introduction-scorecard-audit', {
    method: 'POST',
    headers: { Authorization: `Bearer ${cronSecret}` },
    signal: AbortSignal.timeout(310_000),
  });
  if (!response.ok) throw new Error(`Production introduction scorecard audit returned HTTP ${response.status}`);
  const result = await response.json() as Record<string, unknown>;
  console.log(JSON.stringify({ productionIntroductionScorecardAudit: result }));
}
