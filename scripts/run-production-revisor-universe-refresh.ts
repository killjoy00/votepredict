...
async function main(): Promise<void> {
  ...
  const response = await fetch('https://votepredict.vercel.app/api/operations/revisor-universe-refresh', {
    method: 'POST',
    headers: { Authorization: `Bearer ${cronSecret}` },
    signal: AbortSignal.timeout(310_000),
  });
  if (!response.ok) throw new Error(`Production Revisor universe refresh returned HTTP ${response.status}`);
  const result = await response.json() as Record<string, unknown>;
  console.log(JSON.stringify({ productionRevisorUniverseRefresh: result }));
}
