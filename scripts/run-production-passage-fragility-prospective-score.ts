...
async function main(): Promise<void> {
  ...
  const outputPath = resolve(
    process.env.VOTEPREDICT_PASSAGE_FRAGILITY_SCORE_OUTPUT ?? 'artifacts/passage-fragility-prospective-score-v1.json',
  );
  ...
  const response = await fetch('https://vote.planitnow.us/api/operations/passage-fragility-shadow/score', {
    method: 'POST',
    headers: { Authorization: `Bearer ${cronSecret}` },
    signal: AbortSignal.timeout(310_000),
  });
  if (!response.ok) throw new Error(`Production passage fragility score returned HTTP ${response.status}`);
