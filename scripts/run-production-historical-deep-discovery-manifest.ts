...
async function main(): Promise<void> {
  ...
  const outputPath = resolve(
    process.env.VOTEPREDICT_DEEP_DISCOVERY_OUTPUT ?? 'artifacts/historical-deep-discovery-manifest.json',
  );
  ...
  const response = await fetch('https://vote.planitnow.us/api/operations/historical-deep-discovery-manifest', {
    method: 'POST',
    headers: { Authorization: `Bearer ${cronSecret}` },
    signal: AbortSignal.timeout(310_000),
  });
  if (!response.ok) {
    throw new Error(`Production historical Deep discovery manifest returned HTTP ${response.status}`);
  }
  const parsed = await response.json() as {
    metadata?: { cases?: number; memberCasePairs?: number };
    cases?: Array<{ identifier?: string; occurredOn?: string; members?: unknown[]; currentDeepTargetIds?: unknown[] }>;
  };
  ...
  console.log(JSON.stringify({
    ...
    membersPerCase: parsed.cases?.map((item) => ({
      identifier: item.identifier ?? null,
      occurredOn: item.occurredOn ?? null,
      members: item.members?.length ?? 0,
      currentDeepTargets: item.currentDeepTargetIds?.length ?? 0,
    })) ?? [],
  }));
}
