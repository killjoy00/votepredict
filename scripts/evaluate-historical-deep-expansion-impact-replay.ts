...
async function main(): Promise<void> {
  ...
  const outputPath = resolve(process.argv[7] ?? 'artifacts/historical-deep-expansion-impact-replay.json');
  ...
  const summary = (name: 'current-targets' | 'need-only-targets' | 'discovery-all') => {
  ...
  console.log(JSON.stringify({
    ...
    currentTargets: summary('current-targets'),
    needOnlyTargets: summary('need-only-targets'),
    discoveryAll: summary('discovery-all'),
  }));
}
