...
async function main(): Promise<void> {
  ...
  const outputPath = resolve(process.env.VOTEPREDICT_PASSAGE_FRAGILITY_OUTPUT ?? 'artifacts/passage-fragility-screen-v1.json');
  ...
  const response = await fetch('https://vote.planitnow.us/api/operations/passage-fragility-screen', {
    method: 'POST',
    headers: { Authorization: `Bearer ${cronSecret}` },
    signal: AbortSignal.timeout(310_000),
  });
  if (!response.ok) throw new Error(`Production passage fragility screen returned HTTP ${response.status}`);
  ...
  console.log(JSON.stringify({
    ...
    events: primary?.events ?? null,
    failedEvents: primary?.failedEvents ?? null,
    highConfidenceEvents: primary?.highConfidenceEvents ?? null,
    highConfidenceFailures: primary?.highConfidenceFailures ?? null,
    featureSignals: primary?.featureSignals ?? null,
    riskScoreSignals: primary?.riskScoreSignals ?? null,
