...
async function main() {
  ...
  const { publicPageMentionsPerson } = await import('../src/evidence/public-http.js');
  const { discoverWaybackCaptures, fetchWaybackSnapshot } = await import('../src/evidence/wayback.js');
  const {
    LOCAL_TRADE_NEWS_HISTORY_VERSION,
  ...
  try {
    ...
    const memberRows = await pool.query<{ name: string }>(`
      SELECT DISTINCT l.name
        FROM legislators l
        JOIN memberships m ON m.legislator_id=l.id
        JOIN legislative_sessions s ON s.id=m.session_id
       WHERE s.slug IN ('2021-2022','2023-2024','2025-2026')
       ORDER BY l.name`);
    const memberNames = memberRows.rows.map(row => row.name).filter(Boolean);
    ...
    const result = {
      ...
      policy: {
        ...
        publisherDatePreservedButNotUsedAsArchiveAvailability: true,
        sameDayEligible: false,
        contextOnly: true,
        mechanicallyActionable: false,
        articleInfersLegislativeStance: false,
    ...
    console.log(JSON.stringify({ localTradeNewsArchiveBackfill: result }, null, 2));
  } finally {
    await pool.end();
  }
}
