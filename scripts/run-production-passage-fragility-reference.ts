...
async function main(): Promise<void> {
  ...
  const outputPath = resolve(process.env.VOTEPREDICT_PASSAGE_FRAGILITY_REFERENCE_OUTPUT ?? 'artifacts/passage-fragility-reference-v1.json');
  ...
  const response = await fetch('https://vote.planitnow.us/api/operations/passage-fragility-reference', {
    method: 'POST',
    headers: { Authorization: `Bearer ${cronSecret}` },
    signal: AbortSignal.timeout(310_000),
  });
  if (!response.ok) throw new Error(`Production passage fragility reference returned HTTP ${response.status}`);
