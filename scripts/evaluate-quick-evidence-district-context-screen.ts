...
async function main() {
  ...
  const response = await fetch('https://votepredict.vercel.app/api/operations/quick-evidence-district-context-screen', {
    method: 'POST',
  ...
  if (!response.ok) {
    throw new Error(`Quick Evidence district-context screen returned HTTP ${response.status}: ${body.slice(0, 1800)}`);
  }
  const artifact = JSON.parse(body) as Record<string, unknown>;
  const output = resolve(
    process.env.VOTEPREDICT_QUICK_EVIDENCE_DISTRICT_CONTEXT_OUTPUT
      ?? 'artifacts/quick-evidence-district-context-screen-v1.json',
  );
