...
async function main() {
  ...
  const response = await fetch('https://votepredict.vercel.app/api/operations/quick-evidence-bill-context-screen', {
    method: 'POST',
    headers: { Authorization: `Bearer ${secret}` },
    signal: AbortSignal.timeout(310_000),
  });
  const body = await response.text();
  if (!response.ok) throw new Error(`Quick Evidence bill-context screen returned HTTP ${response.status}: ${body.slice(0, 1800)}`);
  const artifact = JSON.parse(body) as Record<string, unknown>;
  const output = resolve(
    process.env.VOTEPREDICT_QUICK_EVIDENCE_BILL_CONTEXT_OUTPUT
      ?? 'artifacts/quick-evidence-bill-context-screen-v1.json',
  );
  ...
  console.log(JSON.stringify({
    ...
    descriptiveStability: artifact.descriptiveStability,
