...
async function main(): Promise<void> {
  ...
  const outputPath = resolve(
    process.env.VOTEPREDICT_DEEP_EXPANSION_OUTPUT ?? 'artifacts/historical-deep-expansion-cohort.json',
  );
  ...
  const response = await fetch('https://vote.planitnow.us/api/operations/historical-deep-expansion-cohort', {
    method: 'POST',
    headers: { Authorization: `Bearer ${cronSecret}` },
    signal: AbortSignal.timeout(310_000),
  });
  if (!response.ok) {
    throw new Error(`Production historical Deep expansion cohort returned HTTP ${response.status}`);
  }
