...
async function main(): Promise<void> {
  ...
  const outputPath = resolve(
    process.env.VOTEPREDICT_DEEP_TARGET_AUDIT_OUTPUT ?? 'artifacts/historical-deep-target-audit.json',
  );
  ...
  const response = await fetch('https://vote.planitnow.us/api/operations/historical-deep-target-audit', {
    method: 'POST',
    headers: { Authorization: `Bearer ${cronSecret}` },
    signal: AbortSignal.timeout(310_000),
  });
  if (!response.ok) {
    throw new Error(`Production historical Deep target audit returned HTTP ${response.status}`);
  }
  const parsed = await response.json() as {
    overall?: Record<string, unknown>;
    missedHighConfidenceErrors?: unknown[];
  };
  ...
  console.log(JSON.stringify({
    ...
    missedHighConfidenceExamples: parsed.missedHighConfidenceErrors?.length ?? 0,
  }));
}
