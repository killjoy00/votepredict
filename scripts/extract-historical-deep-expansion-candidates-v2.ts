...
function main(): void {
  ...
  const outputPath = resolve(
    argumentValue('--output')
      ?? process.env.VOTEPREDICT_DEEP_EXPANSION_CANDIDATES_V2_OUTPUT
      ?? 'artifacts/historical-deep-expansion-discovery-candidates-v2.json',
  );
  ...
  console.log(JSON.stringify({
    ...
    extractionRules: Object.fromEntries([...new Set(result.candidates.map((item) => item.extractionRule))]
      .sort()
      .map((rule) => [rule, result.candidates.filter((item) => item.extractionRule === rule).length])),
