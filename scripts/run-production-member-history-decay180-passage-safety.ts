...
async function main(): Promise<void> {
  ...
  const outputPath = resolve(
    process.env.VOTEPREDICT_DECAY180_PASSAGE_SAFETY_OUTPUT
      ?? 'artifacts/member-history-decay180-passage-safety-v1.json',
  );
  ...
  const response = await fetch('https://vote.planitnow.us/api/operations/member-history-decay180-passage-safety', {
    method: 'POST',
    headers: { Authorization: `Bearer ${cronSecret}` },
    signal: AbortSignal.timeout(310_000),
  });
  if (!response.ok) throw new Error(`Production decay-180 passage safety audit returned HTTP ${response.status}`);
  const parsed = await response.json() as {
    schemaVersion?: string;
    metadata?: { runtimeCodeSha?: string | null; baselineValidation?: unknown };
    primarySafetySlice?: { deltas?: unknown; frozenChecks?: unknown; passage?: { failed?: unknown } };
    decision?: { status?: string; passageSafe?: boolean; productionAction?: string };
  };
  ...
  console.log(JSON.stringify({
    ...
    primaryDeltas: parsed.primarySafetySlice?.deltas ?? null,
    failedPassage: parsed.primarySafetySlice?.passage?.failed ?? null,
    frozenChecks: parsed.primarySafetySlice?.frozenChecks ?? null,
