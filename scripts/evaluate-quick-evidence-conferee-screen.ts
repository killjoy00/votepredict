...
async function main() {
  ...
  const response = await fetch('https://votepredict.vercel.app/api/operations/quick-evidence-conferee-screen', {
    method: 'POST',
    headers: { Authorization: `Bearer ${secret}` },
    signal: AbortSignal.timeout(310_000),
  });
  const body = await response.text();
  if (!response.ok) throw new Error(`Quick Evidence conferee screen returned HTTP ${response.status}: ${body.slice(0, 1800)}`);
  const artifact = JSON.parse(body) as Record<string, unknown>;
  const output = resolve(
    process.env.VOTEPREDICT_QUICK_EVIDENCE_CONFEREE_OUTPUT
      ?? 'artifacts/quick-evidence-conferee-screen-v1.json',
  );
